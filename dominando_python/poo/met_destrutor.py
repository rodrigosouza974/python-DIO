O método destrutor em Python é um método especial chamado `__del__`. Ele é chamado quando um objeto está prestes a ser destruído. O destrutor é útil para liberar recursos que o objeto possa ter adquirido durante sua vida útil, como arquivos abertos, conexões de rede ou outros recursos externos.

### Como Funciona o Método Destrutor

- **Nome do Método**: `__del__`
- **Execução**: Chamado automaticamente quando o contador de referências de um objeto chega a zero, ou seja, quando não há mais referências ao objeto e o Python decide coletar o lixo (garbage collection).

### Exemplo de Uso do Método Destrutor

Vamos criar uma classe que utiliza o método destrutor para fechar um arquivo quando o objeto for destruído.
...
- **Limitações**: Não é garantido que seja chamado imediatamente após o último uso do objeto. Melhor usar `with` statement para gerenciamento de recursos.

O método destrutor pode ser útil, mas devido à natureza do garbage collection em Python, é importante conhecer suas limitações e preferir outras práticas de gerenciamento de recursos quando apropriado.